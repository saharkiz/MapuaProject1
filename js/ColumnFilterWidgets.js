...
(function($) {
	...
	var ColumnFilterWidgets = function( oDataTableSettings ) {
		...
		if ( 'oColumnFilterWidgets' in oDataTableSettings.oInit ) {
			...
			if ( 'bGroupTerms' in oDataTableSettings.oInit.oColumnFilterWidgets && oDataTableSettings.oInit.oColumnFilterWidgets.bGroupTerms ) {
				...
				me.$TermContainer = $( '<div class="col-lg-12 column-filter-widget-selected-terms"></div><br/><br/><br/>' ).hide();
			}
		}

		// Add a widget for each visible and filtered column
		$.each( oDataTableSettings.aoColumns, function ( i, oColumn ) {
			var $WidgetElem = $( '<div class="col-sm-3 column-filter-widget"></div>' );
	...
	*/
	var ColumnFilterWidget = function( $Container, oDataTableSettings, i, widgets ) {
		...
		widget.$Select = $( '<select></select>' ).addClass('form-control widget-' + widget.iColumn).change( function() {
			...
			$TermLink = $( '<a class="col-sm-3 filter-term" href="#"></a>' )
	...
	*/
	ColumnFilterWidget.prototype.fnDraw = function() {
		...
		if ( widget.asFilters.length === 0 ) {
			...
			// Build the menu
			var tempats= widget.oColumn.sTitle;

			tempats= tempats.substring(0, tempats.indexOf("<"));
			widget.$Select.empty().append( $( '<option></option>' ).attr( 'value', '' ).text( tempats ) );
